...
// إنشاء العملاء (استخدام Intents الكود القديم لضمان التشغيل)
...
client1.on("ready", () => {
    ...
    new userAccount(client1, Discord).leveling({
        channel: ch_ar, randomLetters: false, time: 12000, type: "ar"
    });
    new userAccount(client1, Discord).leveling({
        channel: ch_en, randomLetters: false, time: 12000, type: "eng"
    });
});
...
// تتبع الإحصائيات (بناءً على الإرسال)
const trackMessage = (acc, channelId) => {
    const target = acc === 1 ? stats.c1 : stats.c2;
    target.total++;
    if (channelId === ch_ar) target.ar++;
    if (channelId === ch_en) target.en++;
};

client1.on("messageCreate", m => { if(m.author.id === client1.user.id) trackMessage(1, m.channelId); });
client2.on("messageCreate", m => { if(m.author.id === client2.user.id) trackMessage(2, m.channelId); });

// ريستارت تلقائي كل ساعة (كما في كودك القديم)
...
// ===== واجهة الويب الإنجليزية الصافية =====
const app = express();
        ...
        <div class="container">
            ...
            <div class="grid">
                <div class="card">
                    ...
                    <div class="footer-stats"><span>AR: <b id="c1-ar">0</b></span><span>EN: <b id="c1-en">0</b></span></div>
                </div>
                <div class="card">
                    <h3>Account 2</h3>
                    <span class="val" id="c2-total">0</span>
                    <div class="footer-stats"><span>AR: <b id="c2-ar">0</b></span><span>EN: <b id="c2-en">0</b></span></div>
                </div>
            </div>
        </div>
        <script>
